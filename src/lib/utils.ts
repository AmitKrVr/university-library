...
export const formatDate = (date: Date, format: 'short' | 'numeric' = 'short') => {
  if (format === 'numeric') {
...
export const getTrend = (today: number, yesterday: number) => {
  const value = today - yesterday;
  return { value: Math.abs(value), direction: value > 0 ? "up" : value < 0 ? "down" : "up" };
}
